...
import { reviewDiscussion } from '@/lib/discussion-rubric';
...
export default function RevisionRoomPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => toast.error('Room not created', {
  ...
  const joinMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      toast.success('You joined the Revision Room');
    },
    onError: (error) => toast.error('Could not join', {
      description: error instanceof Error ? error.message : 'Try again.',
    }),
  });
  const startMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (roomId && userId) setRoomCache(queryClient, roomId, userId, result);
      toast.success('Revision Room is live');
    },
  });
  const endMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (roomId && userId) setRoomCache(queryClient, roomId, userId, result);
      toast.success('Group review is ready');
    },
  });
  const utteranceMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (roomId && userId) setRoomCache(queryClient, roomId, userId, result);
      reset();
      toast.success('Your explanation was shared');
    },
    onError: (error) => toast.error('Transcript not shared', {
      description: error instanceof Error ? error.message : 'Try again.',
    }),
  });
    ...
    return (
      <main className="pattern-overlay min-h-screen bg-background p-4 sm:p-8">
        <div className="mx-auto max-w-5xl space-y-6">
          <div>
            <Badge className="rounded-full">Study Squad</Badge>
            <h1 className="mt-3 text-3xl font-bold">Choose a Revision Room topic</h1>
            <p className="mt-2 text-muted-foreground">Everyone joins with their own account and microphone.</p>
          </div>
          {catalogPending ? <Loader2 className="h-6 w-6 animate-spin" /> : (
            <div className="grid gap-4 md:grid-cols-2">
              {catalog?.subjects.flatMap((subject) => subject.topics.map((topic) => (
                ...
                >
                  <span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{subject.name}</span>
    ...
    return (
      <main className="pattern-overlay min-h-screen bg-background p-4 sm:p-8">
        <Card className="mx-auto max-w-2xl rounded-[28px]">
          <CardHeader>
            <Badge className="w-fit rounded-full">Create multiplayer room</Badge>
            <CardTitle className="text-3xl">{topic?.name ?? 'Revision Room'}</CardTitle>
            <p className="text-muted-foreground">{topic?.subjectName} · Invite up to four Study Squad members.</p>
          </CardHeader>
          <CardContent className="space-y-5">
            {!squadQuery.data?.squad ? (
              <div className="rounded-2xl border border-dashed p-5 text-sm">Create or join a Study Squad first.</div>
            ) : inviteableMembers.length === 0 ? (
              <div className="rounded-2xl border border-dashed p-5 text-sm">Your squad has no other members yet. You can create the room now and invite them later.</div>
            ) : (
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => navigate('/revision-room')} className="rounded-full">Back</Button>
              <Button
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-background p-6">
        <Card className="max-w-md rounded-[28px]"><CardContent className="space-y-4 p-8 text-center">
          ...
          <p className="text-muted-foreground">{roomQuery.error instanceof Error ? roomQuery.error.message : 'This room could not be loaded.'}</p>
          <Button onClick={() => navigate('/study-squad')} className="rounded-full">Back to Study Squad</Button>
        </CardContent></Card>
      </main>
    );
  }
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-background p-6">
        <Card className="w-full max-w-lg rounded-[28px]"><CardContent className="space-y-5 p-8 text-center">
          ...
          <div><Badge>{room.squadName}</Badge><h1 className="mt-3 text-3xl font-bold">{room.topicName}</h1><p className="mt-2 text-muted-foreground">Hosted by {room.hostName}</p></div>
          <Button onClick={() => joinMutation.mutate()} disabled={!room.canJoin || joinMutation.isPending} className="w-full rounded-full">
            {joinMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Join room
          </Button>
        </CardContent></Card>
      </main>
    );
        ...
        <section className="space-y-5">
          <Card className="rounded-[28px]">
            <CardContent className="flex flex-wrap items-center justify-between gap-4 p-6">
              <div><Badge className="rounded-full">{room.subjectName}</Badge><h1 className="mt-2 text-2xl font-bold">{room.topicName}</h1><p className="text-sm text-muted-foreground">{room.squadName} · hosted by {room.hostName}</p></div>
              <div className="flex items-center gap-2">
                <Button variant="outline" className="rounded-full" onClick={() => { void navigator.clipboard.writeText(window.location.href); toast.success('Room link copied'); }}><Copy className="mr-2 h-4 w-4" />{room.joinCode}</Button>
                {room.status === 'lobby' && room.canManage && <Button className="rounded-full" onClick={() => startMutation.mutate()} disabled={startMutation.isPending}>Start room</Button>}
                {room.status === 'live' && room.canManage && <Button variant="destructive" className="rounded-full" onClick={() => endMutation.mutate()} disabled={endMutation.isPending}>End & review</Button>}
              </div>
            </CardContent>
          </Card>

          {room.status === 'lobby' && (
            <Card className="rounded-[28px]"><CardContent className="p-8 text-center"><Clock3 className="mx-auto h-10 w-10 text-primary" /><h2 className="mt-4 text-2xl font-bold">Waiting in the lobby</h2><p className="mt-2 text-muted-foreground">Each invited member opens their notification and joins with their own account.</p></CardContent></Card>
          )}

          {room.status === 'live' && (
            <Card className="rounded-[28px]"><CardContent className="space-y-6 p-6 sm:p-8">
              <div className="flex items-center justify-between"><div><p className="text-sm font-bold text-muted-foreground">GROUP EXPLANATION</p><h2 className="text-2xl font-bold">Speak from your own microphone</h2></div><div className="rounded-full bg-secondary px-4 py-2 font-mono font-bold">{formatClock(secondsLeft)}</div></div>
              <Progress value={(secondsLeft / room.durationSeconds) * 100} />
              <div className="rounded-[24px] bg-muted p-5">
                <p className="min-h-20 leading-7">{finalTranscript} <span className="text-muted-foreground">{interimTranscript}</span>{!finalTranscript && !interimTranscript && 'Your live transcript appears here. Other members can record at the same time.'}</p>
                {transcriptionError && <p className="mt-3 text-sm text-destructive">{transcriptionError}</p>}
                <div className="mt-4 flex items-center gap-3">
                  {!isRecording ? (
                    <Button onClick={() => void beginRecording()} disabled={utteranceMutation.isPending} className="rounded-full"><Mic className="mr-2 h-4 w-4" />Start my explanation</Button>
                  ) : (
                    <Button onClick={() => void finishRecording()} className="rounded-full"><Square className="mr-2 h-4 w-4" />Finish & share</Button>
                  )}
                  ...
                  <span className="text-xs text-muted-foreground">{micAvailable ? 'Mic active' : 'Mic meter unavailable'}</span>
                </div>
              </div>
            </CardContent></Card>
          )}
          ...
          {room.status === 'ended' && groupReview && (
            <Card className="rounded-[28px]"><CardContent className="space-y-5 p-6 sm:p-8"><div><Badge className="rounded-full">Group review</Badge><h2 className="mt-2 text-2xl font-bold">What your squad covered</h2></div><div className="grid gap-3 sm:grid-cols-3">{groupReview.group.map((item) => <div key={item.id} className="rounded-2xl border p-4"><div className="flex items-center gap-2">{item.verdict === 'covered' ? <CheckCircle2 className="h-5 w-5 text-primary" /> : <Circle className="h-5 w-5 text-muted-foreground" />}<span className="font-bold">{item.name}</span></div><p className="mt-2 text-sm capitalize text-muted-foreground">{item.verdict}</p></div>)}</div></CardContent></Card>
          )}

          <Card className="rounded-[28px]"><CardHeader><CardTitle>Shared transcript</CardTitle></CardHeader><CardContent className="space-y-3">{room.utterances.length === 0 ? <p className="text-sm text-muted-foreground">No explanations shared yet.</p> : room.utterances.map((utterance) => <article key={utterance.id} className="rounded-2xl bg-muted p-4"><div className="mb-2 flex items-center justify-between gap-3"><strong>{utterance.displayName}</strong><span className="text-xs text-muted-foreground">{new Date(utterance.spokenAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></div><p className="leading-7">{utterance.text}</p></article>)}</CardContent></Card>
        </section>

        <aside>
          <Card className="sticky top-6 rounded-[28px]"><CardHeader><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5" /> Participants</CardTitle></CardHeader><CardContent className="space-y-3">{room.participants.map((participant) => <div key={participant.userId} className="flex items-center gap-3 rounded-2xl border p-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-secondary text-xs font-bold">{initials(participant.displayName)}</span><div className="min-w-0 flex-1"><p className="truncate font-semibold">{participant.displayName}{participant.userId === room.hostUserId ? ' · Host' : ''}</p><p className="text-xs capitalize text-muted-foreground">{participant.presence}</p></div>{participant.presence === 'online' ? <Wifi className="h-4 w-4 text-primary" /> : <WifiOff className="h-4 w-4 text-muted-foreground" />}</div>)}</CardContent></Card>
        </aside>
      </div>
    </main>
  );
