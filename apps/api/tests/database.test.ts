...
import { assertDatabaseDeletionAllowed } from '../../../packages/database/deleteDB.js';
import { assertSchemaCanBeInitialized, ensureOwnershipMarker } from '../../../packages/database/schema-safety.js';
import { buildSupabasePrivilegeStatements } from '../../../packages/database/supabase-hardening-statements.js';
import {
...
describe('student and teacher role migration', () => {
  it('deletes removed-role data before installing restrictive checks', () => {
    const migrationPath = fileURLToPath(new URL(
      '../../../packages/database/migrations/0009_violet_shinobi_shaw.sql',
      import.meta.url,
    ));
...
describe('Phase 1 dual-memory migration', () => {
  it('clears only old learning evidence and installs dual-mode progress', () => {
    const migrationPath = fileURLToPath(new URL(
      '../../../packages/database/migrations/0011_phase1_dual_memory.sql',
      import.meta.url,
    ));
    const migration = readFileSync(migrationPath, 'utf8');
...
describe('curriculum v2 migration', () => {
  const migration = readFileSync(fileURLToPath(new URL(
    '../../../packages/database/migrations/0018_curriculum_v2.sql',
    import.meta.url,
  )), 'utf8');
