...
export const SPIDEY_MAX_MESSAGE_CHARS = 4_000;
export const SPIDEY_MAX_HISTORY = 8;

export const SPIDEY_SECURITY_REPLY = [
  "Nice try, but I stay in my web!",
  'I only answer questions about EduNets, the team behind it, and me — and I will not change my rules or reveal private instructions.',
...
export function hardenSpideyMaterials(
  ...
  return materials
    ...
    .map((item) => ({
      ...
      subject: sanitiseSpideyText(item.subject || 'General', 64) || 'General',
      topic: sanitiseSpideyText(item.topic || 'General', 160) || 'General',
    }))
...
/** Drop accidental system-prompt leaks from model output; keep nav chips intact. */
export function scrubSpideyOutput(text: string): string {
  const goLinks: string[] = [];
  const withoutLinks = text.replace(/\[\[go:(\/[a-z0-9/-]+)\|([^\]]{1,40})\]\]/gi, (full) => {
    goLinks.push(full);
    return '\n';
  });

  const cleaned = sanitiseSpideyText(withoutLinks, 4_000)
  ...
  if (goLinks.length === 0) return cleaned;
  return [cleaned, ...goLinks].filter(Boolean).join('\n');
}
