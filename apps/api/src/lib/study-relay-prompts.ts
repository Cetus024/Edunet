...
/** Easy drawable/explainable Chemistry concepts for the first Concept Relay build. */
...
export function getStudyPromptsForSubject(subject: StudyRelaySubject): StudyPrompt[] {
  if (subject === 'chemistry') return CHEMISTRY_STUDY_PROMPTS;
  return [];
}

export function getStudyPromptById(promptId: string): StudyPrompt | null {
  return CHEMISTRY_STUDY_PROMPTS.find((prompt) => prompt.id === promptId) ?? null;
}
